...
export class CommentPage implements OnInit {
  ...
  uid:any;
  users:any;
  constructor(private afs: AngularFirestore, private route : ActivatedRoute, public afAuth: AngularFireAuth, private authService: AuthService) { 
  ...
  addComment() {
    this.commentsRef.add({ content: this.text, rate:this.rate, createdAt: Date.now(), uid: this.afAuth.auth.currentUser.uid})
    this.text = '';
    this.rate=0
  }
